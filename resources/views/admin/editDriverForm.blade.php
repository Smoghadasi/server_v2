        ...
        <div class="col-xl">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ url('admin/editDriver') }}/{{ $driver->id }}"
                        ...
                        <div class="row row-cols-2">
                            <div class="mb-3 col">
                                ...
                                <input id="nationalCode" type="text" placeholder="کدملی"
                                    class="form-control{{ $errors->has('nationalCode') ? ' is-invalid' : '' }}"
                                    name="nationalCode" value="{{ $driver->nationalCode }}" required>
                            ...
                            <div class="mb-3 col">
                                ...
                                <input id="lastName" type="text" placeholder="نام خانوادگی"
                                    ...
                                    value="{{ $driver->lastName }}" required autofocus>
                            ...
                            <div class="mb-3 col">
                                ...
                                <select class="form-select" name="fleet_id">
                                    ...
                                    @foreach ($fleets as $fleet)
                                        <option @if ($driver->fleet_id == $fleet->id) selected @endif
                                            value="{{ $fleet->id }}">
                            ...
                            <div class="mb-3 col">
                                <label class="form-label" for="basic-default-company">تصویر راننده و کارت شناسایی
                                    :</label>
                            ...
                            <div class="mb-3 col">
                                ...
                                <textarea rows="6" name="operatorMessage" class="form-control" placeholder="توضیحات اپراتور"></textarea>
                            </div>

                            <div class="mb-3 col">
                                <label class="form-label" for="basic-default-company">آدرس محل سکونت :</label>
                                <textarea rows="6" name="address" class="form-control" placeholder="آدرس محل سکونت">{{ $driver->address }}</textarea>
                            </div>
                            <div class="mb-3 col">
                                    ...
                                    <div class="row">
                                        <div class="col-md-2 border border-dark border-3 bg-warning">
                                            ...
                                            <input style="font-size: 30px"
                                                ...
                                                value="{{ $driver->vehicleLicensePlatePartD }}" placeholder="ایران"
                                                name="vehicleLicensePlatePartD" type="text">
                                        </div>
                                        <div class="col-md-2 border border-dark border-3 bg-warning">
                                            <input style="height: 60px; font-size: 30px"
                                        ...
                                        <div class="col-md-2 border border-dark border-3 bg-warning">
                                            <select style="height: 60px;font-size: 30px"
                                                class="form-control border-0 text-center text-center text-dark bg-warning"
                                                name="vehicleLicensePlatePartB" type="text">
                                                @foreach ($persianAlphabet as $alphabet)
                                                    <option @if ($driver->vehicleLicensePlatePartB == $alphabet) selected @endif
                    ...
                    <div id="acceptLevel_{{ $driver->id }}" class="modal fade" role="dialog">
                        <div class="modal-dialog">

                            <!-- Modal content-->
                            <form action="{{ route('driver.updateAuthLevel', $driver->id) }}" method="POST"
                                class="modal-content">
                                @csrf
                                @method('PUT')
                                <div class="modal-header">
                                    <h4 class="modal-title">نوع وضعیت:
                                        ...
                                        @elseif ($driver->authLevel == DRIVER_AUTH_SILVER_PENDING)
                                            <span class="badge bg-label-secondary border border-danger"><span
                                                    class="badge bg-label-secondary">سطح نقره ای : </span> در حال
                                                بررسی</span>
                                        @elseif ($driver->authLevel == DRIVER_AUTH_SILVER)
                                            <span class="badge bg-label-secondary">سطح نقره ای</span>
                                        @elseif ($driver->authLevel == DRIVER_AUTH_GOLD_PENDING)
                                            <span class="badge bg-label-warning border border-danger"><span
                                                    class="badge bg-label-warning">سطح طلایی ای: </span> در حال
                                                بررسی</span>
                                        @elseif ($driver->authLevel == DRIVER_AUTH_GOLD)
                                            <span class="badge bg-label-warning">سطح طلایی</span>
                                        @endif
    ...
    <div class="card">
        ...
        <div class="card-body">
            @foreach ($operatorDriverAuthMessages as $operatorDriverAuthMessage)
                <div
                    class="m-2 p-2 alert text-right
                           @if ($operatorDriverAuthMessage->close == true) alert-secondary
@else
                    alert-primary @endif
                    ">
